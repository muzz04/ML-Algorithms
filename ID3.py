...
def evaluate_algorithm(dataset):
    ...
    for i in range(len(folds)):
        ...
        features=['clump','cell_size','cell_shape','adhesion','epithelial','bare_nuclei','chromatin','nucleoli','mitoses']
        tree_clf = DecisionTreeClassifier(X=x, feature_names=features, labels=y)
        a=tree_clf.id3()
        prediction=list()
        for index,row in test_set.iterrows():
            prediction.append(predict(a,row))
        actual=[row['output_cancer'] for index,row in test_set.iterrows()]
